...
# Reads planting_data.csv and generates 2025.html and 2026.html
...
def read_csv(year):
    """Read CSV and return a dict: { bed_name: [(plant_name, season), ...] }
...
def plant_lines(plants):
    """Format a list of (plant, season) tuples as HTML lines."""
    lines = []
...
def build_lower_map(lower):
    """Build the lower garden map HTML."""
    def bed(name, extra_class=""):
        plants = lower.get(name, [])
        return f'''<div class="bed-card {extra_class}">
    ...
    return f'''
      <div class="lower-garden-map">
        ...
        <div class="lower-beds-grid">
          ...
          <!-- Row 2: Beds Five–Six (shorter, span 2 cols each) -->
          <div class="beds-row row-wide">
            {bed("Five")}
            {bed("Six")}
          </div>
          <!-- Row 3: Beds Seven–Eight -->
          <div class="beds-row row-wide">
            {bed("Seven")}
            {bed("Eight")}
          </div>
...
def build_upper_map(upper):
    """Build the upper garden map HTML."""
    def atbed(name):
...
def build_list_view(lower, upper):
    """Build the list view table HTML."""
    rows = ""
...
def build_page(year):
    """Build the full HTML page for a given year."""
...
<body>
  <header>
    <div class="header-inner">
      ...
      <h1>Our Garden</h1>
      <p class="subtitle">A record of what we\'ve grown, year by year</p>
    </div>
  </header>
  ...
  <footer>
    <p>🌻 Growing together, season by season</p>
  </footer>
